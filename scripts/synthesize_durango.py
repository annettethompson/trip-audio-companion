"""
Synthesize the Durango / Animas River episode from script markdown files.
Usage: python scripts/synthesize_durango.py
"""
import asyncio
import re
import sys
...
EPISODE_TITLE = "The Railroad That Never Quit: Durango, the Animas River, and the San Juan Basin"
EPISODE_SLUG = "durango"

SCRIPTS_DIR = Path("outputs/durango_scripts")
SEGMENTS_DIR = Path("outputs/audio_segments_durango")
...
CHAPTERS = [
    ("00_intro.md", "Introduction: The Day the Mountains Give Way"),
    ("01_dsng_railroad.md", "The Railroad That Never Quit"),
    ("02_durango_founding.md", "The City That Was Built on Purpose"),
    ("03_san_juan_basin.md", "The Basin That Swallowed the Sea"),
    ("04_animas_canyon_drive.md", "Reading the Walls on the Way Down"),
    ("05_southern_ute.md", "The Band That Stayed"),
    ("06_fort_lewis.md", "From Fort to School to College"),
    ("07_durango_hot_springs.md", "The Springs That Changed Their Name"),
    ("08_ponderosa_pine.md", "The Tree That Smells Like Candy"),
    ("09_wildlife_durango.md", "Elk, Bears, and the Mud Engineers"),
    ("10_animas_river_ecology.md", "What the River Remembers"),
    ("11_1923_moment.md", "Boarding the K-28 in 1923"),
...
def extract_narration(md_text: str) -> str:
    ...
    for line in lines:
        ...
        if line.strip() == "---":
            continue
        # Skip topic inventory block (between first --- and second ---)
        if "Topic inventory:" in line:
            skip_topic_inventory = True
            continue
        if skip_topic_inventory:
            # End of topic inventory is a blank line followed by ---
            # We detect it by the second horizontal rule already handled above
            # Instead, skip lines that look like inventory items
            if line.startswith("-") and ("HOME" in line or "REFERENCE" in line):
                continue
            elif line.strip() == "":
                # Blank line within inventory; keep skipping until we hit narration
                continue
            else:
                # We've exited the inventory block
                skip_topic_inventory = False
...
async def main():
    console.print("[bold cyan]Trip Audio Companion -- TTS Synthesis (Durango Episode)[/bold cyan]")
    ...
    if not segment_paths:
        console.print("[red]No segments synthesized. Check outputs/durango_scripts/ directory.[/red]")
        return
